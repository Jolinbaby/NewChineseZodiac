...
namespace PlayerControl
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [Header("Player Underwater")]
        [Tooltip("If the character is in the water or not")]
        ...
        private void UnderwaterCheck()
        {
            // set sphere position, with offset
            Vector3 spherePosition = new Vector3(transform.position.x, transform.position.y - GroundedOffset, transform.position.z);
            Underwater = Physics.CheckSphere(spherePosition, GroundedRadius, WaterLayers, QueryTriggerInteraction.Ignore);

            // update animator if using character
            if (_hasAnimator)
            {
                _animator.SetBool(_animIDSwim, Underwater);
            }
        }
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                {
                    // jumpÒôÐ§
                    SoundManager.Instance.OnJumpAudio();
